...
var $blaix$fake_gren_package_node$Fake$Hello$main = $gren_lang$node$Node$defineSimpleProgram(function(env) {
		return $gren_lang$node$Node$endSimpleProgram(A2($gren_lang$core$Task$onError, function(_v0) {
					return $gren_lang$core$Task$succeed(env.stdout);
				}, $gren_lang$core$Stream$writeLineAsBytes$('Hello from my fake gren package!', env.stdout)));
	});
_Platform_export({'Fake':{'Hello':{'init':$blaix$fake_gren_package_node$Fake$Hello$main($gren_lang$core$Json$Decode$succeed({  }))}}});}(this.module ? this.module.exports : this));
this.Gren.Fake.Hello.init({});
}
catch (e)
{
console.error(e);
